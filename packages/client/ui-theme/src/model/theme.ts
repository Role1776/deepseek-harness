/** Pure theme domain: token declarations, built-in definitions, and override validation. */
...
/** Base themes carried by the built-in stylesheets, always registered. */
...
/** Alias tokens every composition may override, with their intended roles. */
export const BUILTIN_INSPECT_TOKENS: readonly ThemeTokenInspection[] = Object.freeze([
...
/**
 * Describe one token registered outside the built-in catalogue.
 * @param name - token name as registered by a theme or override layer.
 * @returns the inspection row, carrying the CSS variable when the name is one.
 */
export function dynamicToken(name: string): ThemeTokenInspection {
  return {
    name,
 ...
 * @param source - layer identity, named in the rejection message.
 * @param tokens - token-name to value pairs to validate.
 * @returns a defensive copy of the validated layers.
 */
export function validateOverrides(source: string, tokens: ThemeTokenOverrides): ThemeTokenOverrides {
